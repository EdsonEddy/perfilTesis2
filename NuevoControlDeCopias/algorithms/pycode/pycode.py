...
class Pycode:
    ...
    def preprocess(self, code_lines: List[str]):
        code_lines = ['def main():\n'] + list(map(lambda s: '    '+s, code_lines[1:-1]))
        code = ''.join(code_lines)
        return code

    def process_results(self, results, pycode_list):
        description = ''
        for index, func_ast_diff_list in results:
            
            description += 'ref: {}'.format(pycode_list[0]) + '\n'
            description += 'candidate: {}'.format(pycode_list[index]) + '\n'
            ...
            description += '{:.2f} % ({}/{}) of ref code structure is plagiarized by candidate.'.format(
                sum_plagiarism_count / float(sum_total_count) * 100,
                sum_plagiarism_count,
                sum_total_count)
        copy_percentage = sum_plagiarism_count / float(sum_total_count) * 100
        return description, copy_percentage

    def get_per_similarity(self):
        try:
            pycode_list = [self.file_name_a, self.file_name_b]
            with open(self.file_name_a, 'r') as f:
                source_code = f.readlines()
                source_code = self.preprocess(source_code)
            with open(self.file_name_b, 'r') as f:
                candidate_code = f.readlines()
                candidate_code = self.preprocess(candidate_code)
            results = detect([source_code, candidate_code], diff_method=UnifiedDiff)
            description, percentage = self.process_results(results, pycode_list)
